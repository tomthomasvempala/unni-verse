...
// ── Table sub-tab ────────────────────────────────────────────────────────────
function PlayersTable({ users, loans }) {
  ...
  const getStats = (uid) => {
    ...
    const lent = active
      .filter((l) => l.lenderId === uid)
      .reduce((s, l) => s + (l.remainingBalance ?? 0), 0)
    const owed = active
      .filter((l) => l.requesterId === uid)
      .reduce((s, l) => s + (l.remainingBalance ?? 0), 0)
    return { lent, owed }
  }
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
      ...
      {sorted.map((u, i) => {
        ...
        return (
          <div
            key={u.id}
            className="grid grid-cols-4 px-4 py-3 border-b border-gray-700/50 last:border-0 hover:bg-gray-700/30 transition-colors"
          >
...
function BalancePie({ users }) {
  ...
  if (data.length === 0)
    return <p className="text-gray-500 text-sm text-center py-12">No balance data</p>
...
// ── Line chart sub-tab — all players on one graph ───────────────────────────
function BalanceLine({ users }) {
  ...
  useEffect(() => {
    ...
    Promise.all(users.map((u) => getDailyBalances(u.id, period).then((rows) => ({ id: u.id, rows }))))
      .then((results) => {
        // Build a map: date → { date, [displayName]: balance }
        const map = new Map()
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-4">
          ...
          {PERIODS.map(({ label, days }) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                period === days ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
              {label}
            </button>
      ...
      {loadingChart ? (
        <div className="h-64 flex items-center justify-center">
          <div className="w-6 h-6 rounded-full border-2 border-gray-700 border-t-purple-400 animate-spin" />
        </div>
      ) : chartData.length === 0 ? (
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-gray-500 text-sm">{nonAdminUsers.length} players</span>
      </div>
      ...
      {tab === 'Table' && <PlayersTable users={nonAdminUsers} loans={loans} />}
      {tab === 'Pie Chart' && <BalancePie users={nonAdminUsers} />}
      {tab === 'Line Chart' && <BalanceLine users={nonAdminUsers} />}
    </div>
  )
}
