...
export default function TransactionItem({ transaction, currentUserId, users }) {
  const config = TYPE_CONFIG[transaction.type] ?? TYPE_CONFIG.admin_transfer
  const Icon = config.icon
  const isPositive = transaction.toId === currentUserId
  ...
  return (
    <div className="flex items-center gap-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {users && <p className="text-gray-600 text-xs">{formattedDate}</p>}
      </div>
      <p className={`text-sm font-bold flex-shrink-0 ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
        {isPositive ? '+' : '-'}
        {CURRENCY_SYMBOL}
